package com.lachhh.lachhhengine.camera {
	...
	public class CameraFlash extends CameraBase {
		...
		public function CameraFlash(main:EmptyMainGame) {
			...
			boundsFOV.height = ResolutionManager.getGameHeight()-yOffset;
			boundsFOV.x = 0;
			boundsFOV.y = 0;
			visualStaticContainer.y = yOffset;
		...
		override public function refresh() : void {
			...
			gameVisualContainer.container.y += yOffset;
		}
		
		public function getCoordInWorld(x:int, y:int):Point {
			var mouseXFromCenter:Number = x - (Screen.width*0.5);
			var mouseYFromCenter:Number = y - (Screen.height*0.5);
		...
		public function isInBoundsMax(actor:Actor, offsetX:int, offsetY:int):Boolean {
			var isInFOVHorizontally:Boolean = Math.abs(actor.px-px) < (boundsMax.width*0.5)+offsetX;
			var isInFOVVertically:Boolean = Math.abs(actor.py-py) < boundsMax.height*0.5+offsetY;
			return (isInFOVHorizontally && isInFOVVertically);
		}
		...
		public function getRandomXInBoundsAwayFromBorder(borderWidth:int):int {
			var w:int = (boundsFOV.width-borderWidth*2);
			return Math.random()*w+borderWidth+boundsFOV.x;
		}
		
		public function getRandomYInBoundsAwayFromBorder(borderHeight:int):int {
			var h:int = (boundsFOV.height-borderHeight*2);
			return Math.random()*h+borderHeight+boundsFOV.y;
		}
