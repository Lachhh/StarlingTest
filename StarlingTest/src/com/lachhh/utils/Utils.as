package com.lachhh.utils {
	...
	public class Utils {
		...
		static public function SetColorUnity(d:DisplayObject, r:Number, g:Number, b:Number, force:Number):void {
			SetColor(d, -255*force, -255*force, -255*force);
		}
		
		static public function SetColorUnityWhite(d:DisplayObject,force:Number):void {
			SetColor(d, 255*force, 255*force, 255*force);
		}
		
		static public function SetColorUnityRedDeath(d:DisplayObject, force:Number):void {
		...
		static public function removeAllComponentExceptRenderer(actor:Actor):void {
			for (var i : int = 0; i < actor.components.length; i++) {
				var component:ActorComponent = actor.components[i] as ActorComponent;
				var componentAsRenderer:RenderComponent = actor.components[i] as RenderComponent;
				if(componentAsRenderer == null) {
					component.destroyAndRemoveFromActor();
					i--;
				}
			}
		}
